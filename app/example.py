...
class Room:
    ...
    def get_info(self)->Dict:
        return {"room_number":self.room_number,"type":self.room_type.name,"price":self.price_per_night,"floor":self.floor,"status":self.status.value,"amenities":self.amenities,"last_cleaned":self.last_cleaned.isoformat()}
...
class Guest:
    ...
    def get_profile(self)->Dict:
        return {"guest_id":self.guest_id,"name":self.name,"email":self.email,"phone":self.phone,"nationality":self.nationality,"loyalty_points":self.loyalty_points,"vip_status":self.vip_status,"total_reservations":len(self.reservation_history)}
...
class Reservation:
    ...
    def add_service(self,service_type:ServiceType,description:str,price:float):
        service={"type":service_type.value,"description":description,"price":price,"timestamp":datetime.now().isoformat()}
    ...
    def make_payment(self,amount:float,method:PaymentMethod)->Dict:
        ...
        payment_record={"amount":amount,"method":method.value,"timestamp":datetime.now().isoformat(),"remaining":remaining}
        if remaining<=0:
            self.status="paid"
    ...
    def get_reservation_details(self)->Dict:
        return {"reservation_id":self.reservation_id,"guest_name":self.guest.name,"room_number":self.room.room_number,"check_in":self.check_in.isoformat(),"check_out":self.check_out.isoformat(),"num_guests":self.num_guests,"total_amount":self.total_amount,"paid_amount":self.paid_amount,"status":self.status,"services_count":len(self.services),"special_requests":self.special_requests}
...
class Hotel:
    ...
    def create_reservation(self,guest_id:str,room_number:str,check_in:datetime,check_out:datetime,num_guests:int)->Optional[Reservation]:
    ...
    def process_check_out(self,reservation_id:str)->Dict:
        if reservation_id in self.reservations:
            ...
            if reservation.check_out_guest():
                ...
                return {"success":True,"total_amount":reservation.total_amount,"loyalty_points_earned":int(reservation.total_amount/10)}
        return {"success":False}
        
    def search_available_rooms(self,room_type:Optional[RoomType]=None,min_price:float=0,max_price:float=float('inf'))->List[Room]:
        available_rooms=[]
    ...
    def get_revenue_report(self)->Dict:
        ...
        return {"total_revenue":self.revenue,"total_expenses":self.expenses,"net_profit":self.revenue-self.expenses,"total_reservations":total_reservations,"completed_reservations":completed_reservations,"pending_revenue":pending_revenue,"occupancy_rate":self.get_occupancy_rate()}
    ...
    def get_staff_performance_report(self)->List[Dict]:
        ...
        for staff in self.staff.values():
            report.append({"staff_id":staff.staff_id,"name":staff.name,"role":staff.role.value,"performance_score":staff.performance_score,"tasks_completed":len(staff.tasks_completed),"monthly_salary":staff.get_monthly_salary_with_bonus()})
    ...
    def get_guest_statistics(self)->Dict:
        ...
        return {"total_guests":total_guests,"vip_guests":vip_guests,"regular_guests":total_guests-vip_guests,"average_loyalty_points":avg_loyalty_points}
...
def generate_invoice(reservation:Reservation)->Dict:
    invoice={"invoice_id":f"INV-{reservation.reservation_id}","guest_name":reservation.guest.name,"room_number":reservation.room.room_number,"check_in":reservation.check_in.strftime("%Y-%m-%d"),"check_out":reservation.check_out.strftime("%Y-%m-%d"),"room_charges":reservation.room.calculate_price_with_season(reservation.is_peak_season,(reservation.check_out-reservation.check_in).days),"service_charges":sum(s["price"]for s in reservation.services),"discount":reservation.room.calculate_price_with_season(reservation.is_peak_season,(reservation.check_out-reservation.check_in).days)*reservation.guest.get_discount_rate(),"tax":0.10,"total_amount":reservation.total_amount,"paid_amount":reservation.paid_amount,"balance_due":reservation.total_amount-reservation.paid_amount}
    return invoice
