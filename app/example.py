...
class DataValidator:
    ...
    def validate_phone(phone:str)->bool:
        ...
        if not phone.startswith('0'):return False
...
class UserManager:
    ...
    def add_user(self,name:str,email:str,age:int,phone:str)->Dict:
        ...
        user={"id":len(self.users)+1,"name":name.strip(),"email":email.lower(),"age":age,"phone":phone,"created_at":datetime.now().isoformat()}
...
class DataProcessor:
    ...
    def filter_outliers(numbers:List[float],threshold:float=2.0)->List[float]:
        """Loại outliers"""
        if len(numbers)<3:return numbers
    ...
    def group_by_range(numbers:List[int],range_size:int=10)->Dict[str,List[int]]:
        """Nhóm số theo range"""
        if not numbers or range_size<=0:return {}
        
        groups={}
        for num in numbers:
            range_key=f"{(num//range_size)*range_size}-{(num//range_size)*range_size+range_size-1}"
            if range_key not in groups:groups[range_key]=[]
